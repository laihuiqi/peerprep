...
async function findMatch(request) {
    ...
    return new Promise(async(resolve) => {
        ...
        try {
            ...
            await new Promise(resolve => setTimeout(resolve, config.waitingDuration));
...
async function addRequestIntoQueue(channel, criteria, request) {
    try {
        ...
        channel.sendToQueue(config.queueName, Buffer.from(message), { expiration: config.matchingDuration });
